...
for query in cases:
    ...
    result=subprocess.run([str(a.binary.resolve()),'plan','--json','--index',str(a.index.resolve()),'--model',str(a.model.resolve()),'--allow-uninstalled',query],capture_output=True,text=True,timeout=300)
    ...
    rows.append(row)
out={'observations':rows,'semantic_accuracy_verified':False,'proposals_executed':False}
